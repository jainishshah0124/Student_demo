...
# engine = pyttsx3.init()
# en_voice_id = r"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Speech\Voices\Tokens\TTS_MS_EN-US_ZIRA_11.0"  # female
# ru_voice_id = r"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Speech\Voices\Tokens\TTS_MS_RU-RU_IRINA_11.0"  # male
...
Screen_Center_Width = int((1100 - Screen_Width) / 2)
Screen_Center_Height = int((800 - Screen_Height) / 2) - 30
...
while True:
	...
	if k == 32:
		if int(current) <= int(total):
		   ...
		   if(len(boxes) != 0):
			   output_folder = "Datasets/{}".format(folder_name)
			   os.makedirs(output_folder, exist_ok=True)
			   file_path = os.path.join(output_folder, "{}.jpg".format(current))
